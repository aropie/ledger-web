...
from .forms import AccountFormSet, RuleModelForm, SubmitForm
...
def journal(request):
    ...
    entries = list(ledger_api.Journal(settings.LEDGER_PATH))

    entry_filter = request.GET.get("filter", "")
    if entry_filter:
        entries = [
            entry for entry in entries if entry_filter.lower() in entry["body"].lower()
        ]
...
def charts(request):
    ...
    csv = ledger_api.Journal(ledger_path).csv(
        ...
        "--monthly",
        "-X",
        settings.LEDGER_DEFAULT_CURRENCY,
    )
    df = pd.read_csv(
        csv,
        header=None,
...
class RuleIndexView(generic.ListView):
    ...
    def get_queryset(self):
        return Rule.objects.filter(user=self.request.user).order_by(Lower("payee"))
