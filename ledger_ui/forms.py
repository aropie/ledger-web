...
class BaseAccountFormSet(BaseFormSet):
    def clean(self):
        ...
        missing_amount_count = [
            form.cleaned_data.get("amount") for form in self.forms
        ].count(None)
...
class SubmitForm(forms.Form):
    template_name = "ledger_ui/form_snippet.html"
    ...
    def top_fields(self):
        return (self[field] for field in ["date", "payee", "note"])
...
class RuleModelForm(forms.ModelForm):
    ...
    def conditions(self):
        return (self[field] for field in ["payee", "note"])

    def actions(self):
        return (self[field] for field in ["new_payee", "new_note", "account"])
